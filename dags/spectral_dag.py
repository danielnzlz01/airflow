...
with DAG(
    ...
    spectral_clustering_task = DockerOperator(
        task_id='spectral_clustering',
        image='airflow-spectral:latest',
        api_version='auto',
        auto_remove=True,
        command=f'python spectral_clustering.py {storage_conn_str} {container_name}',
        docker_url='unix://var/run/docker.sock',
        mount_tmp_dir=False,
        environment={
            'AZURE_STORAGE_CONNECTION_STRING': storage_conn_str,
...
load_data_task >> scale_data_task >> spectral_clustering_task >> [plot_pca_task, plot_clusters_task, plot_pairplot_task] >> calculate_cluster_means_task