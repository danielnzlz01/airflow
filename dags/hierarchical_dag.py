...
with DAG(
    ...
    load_data_task = DockerOperator(
        task_id='load_data',
        image='airflow-hierarchical:latest',
        api_version='auto',
        auto_remove=True,
    ...
    agglomerative_clustering_task = DockerOperator(
        task_id='agglomerative_clustering',
        image='airflow-hierarchical:latest',
        api_version='auto',
        auto_remove=True,
        command=f'python agglomerative_clustering.py {storage_conn_str} {container_name}',
        docker_url='unix://var/run/docker.sock',
        mount_tmp_dir=False,
        environment={
            'AZURE_STORAGE_CONNECTION_STRING': storage_conn_str,
    ...
    plot_dendogram_task = DockerOperator(
        task_id='plot_dendogram',
        image='airflow-hierarchical:latest',
        api_version='auto',
        auto_remove=True,
        command=f'python plot_dendogram.py {storage_conn_str} {container_name}',
        docker_url='unix://var/run/docker.sock',
        mount_tmp_dir=False,
        environment={
            'AZURE_STORAGE_CONNECTION_STRING': storage_conn_str,
...
load_data_task >> scale_data_task >> agglomerative_clustering_task >> [plot_dendogram_task, plot_pca_task, plot_clusters_task, plot_pairplot_task] >> calculate_cluster_means_task